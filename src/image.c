...
int load_image( struct image* image, const char* filepath )
{
    ...
    image->impl =
        SDL_CreateTexture( screen->impl, SDL_PIXELFORMAT_RGBA8888,
...
void draw_image( struct image* image, int x, int y, struct rectangle* clip,
                 double angle )
{
    ...
    render_quad.x = x; render_quad.y = y;
    render_quad.w = image->width; render_quad.h = image->height;

    if ( clip != NULL ) {
        render_quad.w = clip->w;
        render_quad.h = clip->h;
        sdl_clip.x = clip->x; sdl_clip.y = clip->y;
        sdl_clip.w = clip->w; sdl_clip.h = clip->h;
...
struct image* create_target_image( int w, int h, struct color color )
{
    ...
    if ( image != NULL ) {
        ...
        SDL_SetRenderDrawColor( screen->impl, color.red, color.green,
